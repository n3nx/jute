...
mod tests {
    ...
    fn verify_ps256() {
        ...
        let signature_bytes: Vec<u8> = not_err!(Base64::decode_vec(signature));
        let public_key = Secret::public_key_from_file("test/fixtures/rsa_public_key.der").unwrap();
        not_err!(SignatureAlgorithm::PS256.verify(
    ...
    fn verify_es256() {
        ...
        let public_key = Secret::PublicKey(not_err!(hex::decode(public_key)));
        ...
        let signature_bytes: Vec<u8> = not_err!(hex::decode(signature));
        not_err!(SignatureAlgorithm::ES256.verify(
            signature_bytes.as_slice(),
            &payload_bytes,
            &public_key,
